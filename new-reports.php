	...
	if($_SERVER['REQUEST_METHOD']=='POST')
	{
		...
		if($selected && is_array($selected) && count($selected) > 0)
		{
			...
			if(isset($_POST['archiveit']))
			{
				...
				{
					if(!$wprp->archive($reportid, $current_user->ID, $archive_c))
					{
	...
	<table class="widefat post fixed" cellspacing="0">
		...
		foreach($results as $report):
			...
			<tr <?php echo $alt;?>>
				...
				<td><a href="<?php echo $permalink;?>" title="<?php echo ($report->commentID > 0 ?'Edit The Comment':'Edit The Post');?>"><?php echo $report->post_title.($report->commentID>0?' (Comment Reported)':' (Post Reported)');?></a></td>
